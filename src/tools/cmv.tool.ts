...
type FetchCmvReport = (path: string, database: string) => Promise<{ url: string; expiresAt: string }>;

const dbParam = { database: z.string().optional().describe('Banco da empresa (padrão: crmoema)') };

export function registerCmvTools(ctx: ToolContext, fetchCmvReport: FetchCmvReport) {
