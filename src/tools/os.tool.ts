...
export function registerOsTools({ server, pool, authorize }: ToolContext) {
  server.tool(
    ...
    async ({ id }) => {
      ...
      if (!rows.length) {
        return { content: [{ type: 'text' as const, text: `OS ${id} não encontrada.` }] };
      }
  ...
  server.tool(
    ...
    async ({ id }) => {
      ...
      if (!rows.length) {
        return { content: [{ type: 'text' as const, text: `Tipo de serviço ${id} não encontrado.` }] };
      }
  ...
  server.tool(
    ...
    async ({ id }) => {
      ...
      if (!rows.length) {
        return { content: [{ type: 'text' as const, text: `Status ${id} não encontrado.` }] };
      }
