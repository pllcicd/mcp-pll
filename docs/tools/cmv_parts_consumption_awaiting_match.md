# Ferramenta: cmv_parts_consumption_awaiting_match

**Nome interno:** `cmv_parts_consumption_awaiting_match`
**Título de exibição:** `Relatório: Consumo de Peças Aguardando Casamento`
...
Gera o relatório de **Consumo de Peças Aguardando Casamento** (CMV) na API do Grupo PLL e retorna um link de download `.xlsx` válido por tempo limitado.
...
```
GET https://api.grupopll.com.br/nasajon/reports/cmv/parts-consumption-awaiting-match?database=crmoema
...
```json
{
  ...
  "method": "tools/call",
  "params": {
    "name": "cmv_parts_consumption_awaiting_match",
